 ...
 *
 */
#define MAX_PERIODS (MOUSE_JIGGLER_SECONDS_MAX)
#define MOUSE_JIGGLER_PPERIOD (MOUSE_JIGGLER_SECONDS_MAX-MOUSE_JIGGLER_SECONDS_MIN)

// WPM Stuff
static uint8_t  current_mouse_jiggler = 0;
static uint32_t mouse_jiggler_timer   = MOUSE_JIGGLER_SECONDS_MAX;
 ...
 */
#    define LATENCY (100)
static uint8_t  prev_mouse_jiggler        = 0;
static uint8_t  next_mouse_jiggler        = 0;
#endif

bool mouse_jiggler_keycode(uint16_t keycode) {
    return mouse_jiggler_keycode_kb(keycode);
}

__attribute__((weak)) bool mouse_jiggler_keycode_kb(uint16_t keycode) {
    return mouse_jiggler_keycode_user(keycode);
}

__attribute__((weak)) bool mouse_jiggler_keycode_user(uint16_t keycode) {
...
__attribute__((weak)) uint8_t mouse_jiggler_regress_count(uint16_t keycode) {
...
// unregistering (pressing and releasing) `MOUSE_JIGGLER_KEYCODE`.
static uint32_t mouse_jiggler_callback(uint32_t trigger_time, void* cb_arg) {
  if (click_registered) {
    unregister_code16(MOUSE_JIGGLER_KEYCODE);
    click_registered = false;
  } else {
    click_registered = true;
    register_code16(MOUSE_JIGGLER_KEYCODE);
  }
  return MOUSE_TURBO_CLICK_PERIOD / 2;  // Execute again in half a period.
}

void update_mouse_jiggler(uint16_t keycode) {
    if (mouse_jiggler_keycode(keycode) && period_presses[current_period] < INT16_MAX) {
        period_presses[current_period]++;
    }
#if defined(WPM_ALLOW_COUNT_REGRESSION)
    uint8_t regress = mouse_jiggler_regress_count(keycode);
...
void decay_mouse_jiggler(void) {
    ...
    mouse_jiggler_timer = (MOUSE_JIGGLER_SECONDS_MAX - (rand() % MOUSE_JIGGLER_PPERIOD))*1000;
    int32_t  elapsed  = timer_elapsed32(mouse_jiggler_timer);
    uint32_t duration = (((periods)*PERIOD_DURATION) + elapsed);
    int32_t  mouse_jiggler_now  = (60000 * presses) / (duration * WPM_ESTIMATED_WORD_SIZE);

    if (mouse_jiggler_now < 0) // set some reasonable WPM measurement limits
        mouse_jiggler_now = 0;
    if (mouse_jiggler_now > 240) mouse_jiggler_now = 240;
...
#if defined(WPM_UNFILTERED)
    current_mouse_jiggler = mouse_jiggler_now;
#else
    int32_t latency = timer_elapsed32(smoothing_timer);
    if (latency > LATENCY) {
        smoothing_timer = timer_read32();
        prev_mouse_jiggler        = current_mouse_jiggler;
        next_mouse_jiggler        = mouse_jiggler_now;
    }

    current_mouse_jiggler = prev_mouse_jiggler + (latency * ((int)next_mouse_jiggler - (int)prev_mouse_jiggler) / LATENCY);
#endif
}
